...
\textit{Ce chapitre, grâce aux notions introduites au Chapitre~\ref{sec:sinusoidal}, permet de porter un regard nouveau sur l'analyse des circuits électriques en utilisant les concepts de «~Signaux et Systèmes~», qui sont d'abord brièvement rappelés, puis particularisés à des circuits simples et enfin illustrés sur l'exemple de la radio AM.}
...
\begin{testexample}[Impédance d'un circuit RL série.]
	...
	De retour dans le domaine temporel, on peut écrire : 
	\begin{align*}
...
\paragraph{Filtre.}
Un filtre est un circuit qui permet d'éliminer ou d'atténuer fortement certaines
...
Les circuits résonants sont des circuits du second ordre, constitués d'au moins une bobine et d'un condensateur.
On peut grouper les nombreuses applications des circuits résonants en deux catégories.
...
oscillant en tant qu'adaptateur d'impédances\footnote{Remarquons que le circuit résonant est en fait un quadripôle.}.  Ces deux fonctions peuvent être remplies par le circuit résonant série aussi bien que parallèle. Nous abordons uniquement leur utilisation en tant que filtre dans cette section, et l'illustrons directement sur l'exemple de la radio AM.
...
\subsection{Antenne}
Tout d'abord, il faut capter un signal. Ce signal se trouve dans l'air ambiant 
et est capté par un simple fil. Celui-ci va en réalité capter des différences 
de potentiel dues à la présence des ondes électromagnétiques qu'on souhaite 
capter. Ainsi, plus le fil est long, meilleure sera la réception. L'antenne 
peut être vue comme un générateur de courant connecté entre la masse du circuit 
et la connexion physique de l'antenne. 
\mytodo{Comme dans Hurwitz(?Howatson? plutôt), montrer une mesure du signal reçu par l'antenne sans et puis avec le filtre.}

\subsection{Circuit résonant}
Le signal capté par l'antenne doit ensuite être filtré. 
L'antenne capte une multitude de signaux mais seulement une partie nous intéresse. 
Nous cherchons à ne faire passer qu'une fréquence comprise entre 500 et 1600 kHz 
en AM. Pour «~sélectionner~» le signal porteur qui nous intéresse, nous utilisons 
un filtre passe-bande fait d'un dipôle LC parallèle. 
...
On peut par exemple choisir de changer l'inductance de la bobine en utilisant 
un noyau en ferrite~: en le faisant coulisser dans la bobine, on change la 
perméabilité magnétique et donc on change L. 
%Pour notre circuit, avec une capacité fixe $C=470$ pF, on a $L\in [21, 184]$ $\mu$H. 
On peut également faire varier C, en utilisant un condensateur à capacité variable. 
Dans notre circuit exemple, pour un L fixé à $4.7$ mH, on a $C \in [10, 220]$ pF. 

% Pour pouvoir tester ces deux possibilités, nous avons ajouté un switch dans le 
% circuit qui permet de basculer vers un dipôle L -- C variable ou vers un dipôle 
% L variable -- C. 

À la fréquence de résonance, l'impédance du dipôle est (en théorie) infinie,
en pratique les imperfections des composants créent une impédance très grande.
Les moindres petits courants captés aux fréquences avoisinant celle de la 
porteuse se développent donc dans cette grande impédance et créent ainsi un signal 
en tension suffisamment grand pour être manipulé par la suite du circuit.  
Pour des fréquences non voulues, l'impédance sera au contraire très petite et 
laissera passer les signaux à travers soit l'inductance $L$ ($f<f_0$) soit 
le condensateur $C_{var}$ ($f>f_0$) sans générer de tension notable.  

\subsection{Masse de la Radio}
Il ne faut pas oublier de définir un point de potentiel de référence pour notre 
système. Dans notre cas, la borne '-' d'une pile 9~V a été choisie comme notre potentiel 
de référence. Attention à ne pas confondre la \textit{masse} et la \textit{prise de terre} 
d'un circuit. La \textit{masse} d'un appareil est plus généralement le potentiel
de référence du circuit considéré comme étant à la tension 0 \volt. La 
\textit{prise de terre} désigne dans les infrastructures électriques 
(maisons, bâtiments, pylônes électriques, etc.) un conducteur physiquement 
connecté au sol. Elle est parfois choisie comme \textit{masse} d'un appareil 
branché dans une prise, mais ce n'est pas toujours le cas.
Notre radio étant portable, une des bornes de la pile est un bon candidat pour 
...
Une façon alternative de comprendre la détection d'enveloppe 
(cf. Section~\ref{demodu2}), après la rectification du signal, 
est de considérer le circuit de la coupe 2 de la Figure~\ref{fig:RadioAM_diode} 
comme un filtre passe-bas avec une fréquence de coupure légèrement supérieure 
à $20 \kHz$. 

\subsection{Filtre passe-haut pour le haut-parleur}
Comme on le verra au Chapitre~\ref{cha:AO}, le deuxième amplificateur 
opérationnel ajoute un \textit{offset} de $4{,}5\text{ V}$ au signal. 
Cependant, un haut-parleur ne peut recevoir qu'un signal 
alternatif, sans composante continue qui lui serait nuisible. 
Il faut donc retirer cet \textit{offset} (composante continue) 
et c'est à cela que sert le condensateur $C_5$ série précédant le 
haut-parleur (coupe 6 de la Figure~\ref{fig:RadioAM_filtres}). 
Utilisé comme filtre passe-haut, il va éliminer l'\textit{offset} qui est de 
fréquence nulle en théorie et de fréquence très faible en pratique.

\subsection{Filtre passe-haut avant amplification}
Dans la même idée, le condensateur $C_2$ et la résistance $R_2$ forment un 
filtre passe-haut du premier ordre. Ce dernier élimine toute composante DC qui 
pourrait s'introduire dans le circuit avant l'amplification. En effet, il est 
important de rappeler que nous sommes entourés de signaux électromagnétiques et 
que chaque morceau de conducteur du circuit peut former une antenne. 
Si un signal parasite basse fréquence s'introduit dans le circuit après le circuit 
résonant et qu'on ne le filtre pas, il pourrait alors être amplifié et «~manger~»
la part du signal audio. 
On dit qu'on filtre ces parasites à très basse 
fréquence pour éviter que l'amplificateur ne sature. La raison
de cette saturation sera couverte au Chapitre~\ref{cha:AO}.  
