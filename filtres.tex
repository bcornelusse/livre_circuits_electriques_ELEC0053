...
peut être vu comme un générateur de courant connecté entre la masse du circuit 
...
%Pour notre circuit, avec un capacité fixe $C=470$ pF, on a $L\in [21, 184]$ $\mu$H. 
...
\`A la fréquence de résonance, l'impédance du dipôle est (en théorie) infinie,
en pratique les imperfections des composants crèe une impédance très grande.
Les moindre petits courants captés aux fréquences avoisinnant celle de la 
porteuse se développent donc dans cette grande impédance et crée ainsi un signal 
...
système. Dans notre cas la borne '-' d'une pile 9V a été choisie notre potentiel 
...
de référence du circuit considéré comme étant à la tension 0V. La 
\textit{prise de terre} désigne dans les infrastructures électriques 
(maisons, bâtiments, pylones électrique, etc.) un conducteur physiquement 
connecté au sol. Elle est parfois choisie comme \textit{masse} d'un appareil 
branché dans une prise d'où la confusion mais ce n'est pas toujours le cas.
Notre radio étant portable, une des bornes de la pile est un bon candidat pour 
potentiel de référence. 
%Les signaux non voulus mentionné précedemment circule donc 
 

%Il s'agit d'un circuit de retour pour le courant. Un courant ne se 
%créera donc dans l'antenne que si celle-ci est reliée au sol. 
%Cette terre permettra donc de "réceptionner" les signaux non voulus qui passent 
%à travers le filtre. 
%Dans le cadre de cette expérience, nous relierons ce point de référence à la 
%terre d'une prise électrique.
...
comme un filtre passe bas avec une fréquence de coupure légèrement supérieur 
à $20\si{\kilo\hertz}$. 

\subsection{Filtre passe-haut pour le haut-parleur}
Comme on le verra au chapitre~\ref{cha:AO}, le deuxième amplificateur 
opérationnel ajoute un \textit{offset} de $4,5$ V au signal. 
...
Dans la même idée, le condensateur $C_2$ et la résistance $R_2$ forme un 
filtre passe haut de premier ordre. Ce dernier élimine toute composante DC qui 
pourrait s'introduire dans le circuit avant l'amplification. En effet, il est 
important de rappelé que nous sommes entourés de signaux électromagnétiques et 
que chaque morceaux de conducteurs du circuit peut former une antenne. 
Si un parasite basse fréquence s'introduit dans le circuit entre le circuit 
résonant et qu'on ne le filtre pas, il pourrait alors être amplifier et manger
la part du signal audio en quelque sorte. 
On dit qu'on filtre ces parasites très basses 
