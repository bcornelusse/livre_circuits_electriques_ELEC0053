
\section{Exercice non résolu}
\begin{exercise}{}
On considère le quadripôle suivant.
\begin{center}
	\includegraphics[width=0.5\textwidth]{exercices/ex-4-6} \\
	$C_1=15\text{ nF}$~; $C_2=5\text{ nF}$~; $L=0.1\text{ mH}$~; $R = \frac{400}{3}\ \Omega$ 
\end{center}
Déterminer~:
\begin{enumerate}
	\item la réponse fréquentielle 
	\[H(j\omega)=\left.\frac{\bar{U}_2}{\bar{I}_1}\right|_{\bar{I}_2=0}\]
	\item ses fréquences naturelles et ses zéros de transmission.
\end{enumerate}
\rep{
	$H(j\omega)= \frac{-\omega^2+2\, 10^{12}}{15\, 10^{-9}\left(
		-j\omega^3 - 2\, 10^6\, \omega^2 + 2\, 10^{12} \, j\omega + 10^{18}\right)}$
}
\end{exercise}