...
\begin{exwithsol}{Phaseurs.}{solex:phaseur-1}
	...
	\item Les placer sur un diagramme de phaseurs. 
...
\begin{exwithsol}{}{solex:RSE_1}\label{ex:RSE_1} 
	...
	Tracer le diagramme de phaseurs et illustrer également $\bar{V}_{12}$ et $\bar{V}_{23}$.
	\begin{center}
...
Une résistance de $5\text{ k}\Omega$ est connectée en série avec la
...
$R_1 =2\text{ k}\Omega$ ; $R_2 =4\text{ k}\Omega$ ; $L_1 =10\text{ mH}$ ; $\bar{E}=120\angle 0\text{ V}$, valeur efficace
\end{center}
...
$R_1 =2\text{ k}\Omega$ ; $R_2 =4\text{ k}\Omega$ ; $L_1 =10\text{ mH}$ ; $\bar{E}=120\angle 0\text{ V}$, valeur efficace ; $Z = 5+j5\text{ k}\Omega$
\end{center}
...
\rep{$\omega = 9.95\, 10^4$ rad/s, $Z_L=500\,\,\Omega$ ; 
	$P_Z = 0.575$ W, $S_E=8.3-j15.5$ VA}
\end{exwithsol}

\begin{exwithsol}{}{solex:2-10}
	\label{ex:2-10}
	Un générateur délivrant une f.e.m. $e(t)=E\cos \omega t$ est
	connecté, depuis un temps supposé infini, à un circuit $\cal R$
	...
	mise en série d'une résistance $R$ et d'une inductance $L$. À
...
\begin{center}
	...
	$R_1=100\,\, \Omega$, $R_2=1\text{ k}\Omega$, $C=5\,\, \mu\mbox{F}$, $L = 2 \,\, \mbox{mH} $, $\bar{E}=10\angle 0\,\, \mbox{V} \mbox{~(valeur efficace)}$
\end{center}
...
%\rep{$\bar{I}_c = -72.11 \angle 106^{\circ}$ A} % magnitude négative ça marche pas en +
\rep{$\bar{I}_c = 124.9 \angle 103^{\circ}$ A (240 = phase, A=0°)\\
	 ou $\bar{I}_c = 72.1\angle 133^{\circ}$ A (240 = ligne,  AC=0°)\\
	 ou $\bar{I}_c = 72.1\angle 73^{\circ}$ A (240 = ligne,  AB=0°)} %doublement vérifié avec Ltspice
...
\begin{eqnarray*}
	G' &=& G\, (1+Q^2) \: \simeq \: Q^2 G ~~\mbox{si} ~~Q \gg 1\\
	C' &=& C \left( 1 + \dfrac{G^2}{\omega^2 C^2} \right) \: = 
	\: C \left( 1 + \dfrac{1}{Q^2} \right) \: \simeq \: C ~~\mbox{si} ~~Q \gg 1
\end{eqnarray*}
...
\begin{enumerate}
	...
	\end{enumerate}
	\item De la PLK au nœud $A$, on déduit $\bar{I}_E$~:
...
En transformant la partie gauche du circuit comme suit (par exemple par équivalences de sources et association en parallèle)~:
\begin{center}
...
\begin{align*}
	...
	& = 9.72 \text{~W}
\end{align*}
On déduit la puissance instantanée en $t=10^{-3}\text{ s}$~:
...
\begin{enumerate}
	...
	\begin{enumerate}
		...
		\item le terme $i_{etabli}=B\cos(\omega t + \phi )$, constituant le régime
		...
		fait à la solution du circuit en régime sinusoïdal établi. Son
...
a) Soient $i_1$ et $i_2$ les deux courants parcourant les deux
...
\end{figure}
Étant donnés les sens choisis pour les courants \ $i_1\, , \, i_2$~,
...
\end{eqnarray*}
D'autre part, la PLK écrite au nœud 1 fournit~:
\[ i\,=\; i_1+i_2 \]
et donc
...
et finalement~:
\begin{eqnarray*}
	u &=& \left( L_1 \dfrac{L_2+M}{L_1+M} - M \right)   \, \dfrac{1}{\frac{L_2+M}{L_1+M} + 1} 
	\,  \dfrac{di}{dt}\\
	&=& \dfrac{L_1L_2 - M^2}{L_1 + L_2 + 2M} \,  \dfrac{di}{dt}~.
\end{eqnarray*}
...
Le courant d'établissement de ce circuit est donné par~:
...
Il faut remarquer qu'il n'était pas possible de vérifier directement les valeurs finales de $ i_1$ et $i_2$ sans recourir au calcul des flux. En effet, pour \ $t\rightarrow\infty$, les deux bobines se comportent comme
des courts-circuits, et il n'est pas possible de déterminer comment le courant total de c.c, $i_0$, se divise entre les deux bobines.
