...
\noindent Déterminer le bilan de puissance pour chacun des trois circuits ci-dessous.
\begin{center}
...
\begin{exwithsol}{Association de résistances.}{solex:1-3}
\label{ex:1-3}
...
\begin{exwithsol}{Le diviseur potentiométrique.}{solex:1-4}
\label{ex:1-4}
\begin{enumerate}
	...
	figure ci-dessous.
	\item On alimente une résistance de charge $R_L$ par la tension
...
\begin{exwithsol}{Sources réelles de tension et de courant.}{solex:1-6}
\label{ex:1-6}
\begin{enumerate}
...
\begin{exwithsol}{Équivalence de sources.}{solex:1-7}
\label{ex:1-7}
\begin{enumerate}
...
\begin{exwithsol}{Équivalence de sources et réduction.}{solex:1-8}
\label{ex:1-8}
\begin{enumerate}
...
\begin{exwithsol}{Source commandée.}{solex:1-9}
\label{ex:1-9}
...
La première loi de Kirchhoff (PLK) au nœud a s'écrit~:
...
\[\sum p_{\text{consommées}}=0 \qquad p_E+p_R-p_J=0\]
ou
\[\sum p_{\text{consommées}}=\sum p_{\text{fournies}} \qquad
p_E+p_R=p_J\]
...
\begin{enumerate}
	\item par application de la PLK au nœud b ou c~:
	\[ I_1-I_0-6=0\]
...
\begin{solexercise}{\ref{ex:1-3}}
	\label{solex:1-3}
{\bf Équivalence entre deux dipôles~:}
{\em deux dipôles sont équivalents si pour une même
	d.d.p. aux bornes $v$, ces dipôles sont parcourus par un même courant
...
V_2 &= \frac{R_2}{R_1[1+ (R_2/R_L)]+R_2}V_s
\end{align*}
Nous aurons donc 
...
\begin{solexercise}{\ref{ex:1-7}}
	\label{solex:1-7}
\noindent{\bf 1. Équivalence source de tension $\rightarrow$ source de
	courant.}
Les deux dipôles sont équivalents s'ils délivrent un même courant $I$
sous une même tension $V$.
Le courant débité par le dipôle «~source de tension~» s'écrit~:
	\[I= \frac{V_s}{R_s}-\frac{V}{R_s}\]
Le courant débité par le dipôle «~source de courant~» vaut~:
	\[I=I_s-G_s V\]
La condition d'équivalence est~:
...
\noindent{\bf 2. Équivalence source de courant $\rightarrow$ source de
	tension.}
Les deux dipôles sont équivalents s'ils présentent une même tension $V$
...
\end{center}
En connectant ce dipôle au reste du circuit, on a~:
\begin{center}
...
Le circuit étant remplacé par son dipôle équivalent «~source de tension~», on connecte
...
\end{minipage}\\
\\
\noindent{\bf 3. État électrique complet du circuit.}
...
\end{center}
La PLK appliquée aux nœuds a et b fournit~:
\begin{align*}
I_s&=I_{\phi}+5 = 10\\
