...
\end{center}
La PLK \'ecrite aux nœuds \ $a$ et $b$ \ fournit~:
\begin{eqnarray}
...
\end{eqnarray*}
Les constantes \ $K_1$ et $K_2$ \ se d\'eduisent des conditions initiales pour \ $u_1$ et $u_2$~.
On a~:
\begin{eqnarray*}
...
avec
\begin{eqnarray*}
	\alpha &=& \dfrac{1}{2RC}~, ~~\mbox{le coefficient d'amortissement}\\
...
\end{eqnarray*}
et finalement
\[ v(t) \: = \, -\, 5\, 10^5\, t \, e^{-4000t} + 25\, e^{-4000t}. \]
\end{solexercise}
...
\end{enumerate}
\noindent{\bf 1\textsuperscript{re} période}  : $0\leq t < t_s$
...
\noindent{\bf 2\textsuperscript{e} période}  : $t_s\leq t < \infty$
...
{\bf 1\textsuperscript{re} période~: \ $0\leq t < t_d$} 
...
\vspace{\baselineskip}
{\bf 2\textsuperscript{e} période~: \ $ t \geq  t_d$}
...
o\`u, en fonction de \ $t$, en rempla\c{c}ant \ $t'$ \ par \ $t-t_d$~:
\begin{eqnarray*}
...
La figure ci-dessous compare les deux courants $i(t)$ et $i^{'}(t)$ 
...
\begin{enumerate}
	...
	\item En $t_1 \approx 0.17s$, le courant vaut $-6.65 A$. C'est la condition initiale de cette derni\`ere phase. Si on translate l'origine des temps en $t_1$, on a maintenant une solution de type r\'eponse libre
	$$\frac{di(t)}{dt} + \frac{R}{L} i(t)  = \frac{20}{L},$$
	et donc $i(t) = \frac{20}{R} + K e^{-\frac{t}{\tau}}$ avec $\tau = \frac{L}{R}$, et donc $$K = -6.65 - \frac{20}{R} = -16.65.$$
	Finalement, et en translatant à nouveau l'origine des temps,  $$i(t) = 10 - 16.65 e^{-4(t-t_1)}.$$
\end{enumerate}
